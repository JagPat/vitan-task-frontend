...
async def run_test():
    ...
    try:
        ...
        # Click the Login button to navigate to the login page for OAuth login attempt
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div/div/div[2]/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Input invalid WhatsApp number and attempt login to test error handling
        frame = context.pages[-1]
        ...
        assert False, 'Test failed: OAuth login with invalid credentials did not behave as expected.'
