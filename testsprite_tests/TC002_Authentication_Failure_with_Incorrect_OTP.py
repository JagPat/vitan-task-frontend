...
async def run_test():
    ...
    try:
        ...
        # Enter a valid registered WhatsApp number and request OTP.
        frame = context.pages[-1]
        ...
        # Enter an incorrect OTP and submit it.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div[3]/div[2]/div[2]/form/div[2]/div/div/input').nth(0)
        ...
        # Assert that the login attempt is rejected with an appropriate error message about invalid OTP.
        frame = context.pages[-1]
        error_message_locator = frame.locator('text=Login failed. Please try again.')
        assert await error_message_locator.is_visible(), 'Expected error message about invalid OTP is not visible'
