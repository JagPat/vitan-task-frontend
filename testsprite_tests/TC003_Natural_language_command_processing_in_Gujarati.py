...
async def run_test():
    ...
    try:
        ...
        # Input a valid WhatsApp number and click Login to authenticate
        frame = context.pages[-1]
        ...
        # Click the Login button to attempt login again or close modal if already logged in
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div[3]/div[2]/div[2]/form/div[3]/button[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Send a natural language task-related message in Gujarati via WhatsApp to trigger task creation
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div[3]/main/div/div[4]/div[2]/div/div[2]/a/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Input a natural language task title in Gujarati and select a team member to assign the task
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div[3]/main/div/div/form/div[2]/div[2]/div/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('આજના મીટિંગ માટે પ્રેઝન્ટેશન તૈયાર કરવું')
        ...
        # Clear and re-enter the Gujarati task title correctly, then select a team member from the dropdown to assign the task
        frame = context.pages[-1]
        ...
        # Select a team member from the dropdown to assign the task and then submit the form to create the task
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div[3]/main/div/div/form/div[2]/div[2]/div[2]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Select 'Shailesh Panchal' as the team member and submit the task creation form to create the task
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div[2]/div/div/div[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
